...
func createExternalIndexer(args []string, zipp bool, in io.Reader) int {
	...
	if args[0] == "-nihocc" {
		...
		for scanr.Scan() {
			...
			buffer.WriteString("\t")
			buffer.WriteString(eutils.PadNumericID(scd))
			buffer.WriteString("\n")
			...
			buffer.WriteString("\t")
			buffer.WriteString(eutils.PadNumericID(fst))
			buffer.WriteString("\n")
...
func main() {
	...
	// database argument, currently supports PubMed (default) and PMC (BioC format)
	db := ""
	...
	for {
		...
		// database (currently pubmed or pmc)
		case "-db":
	...
	*/
	...
	if stsh != "" && indx != "" {
		...
		stsq := eutils.CreateStashers(stsh, parent, indx, pfx, sfx, hshv, zipp, asn, pmc, 1000, xmlq)
