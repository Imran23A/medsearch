...
GenerateFromSOLR() {
  ...
  # results are sorted by [EDAT], so first entry is USUALLY the highest
  maxPMID=$(
    esearch -db pubmed -subset "all [SB]" -retmax 1 -tranquil < /dev/null |
    xtract -pattern eSearchResult -element Id
  )
  ...
  then
    ...
    then
      ...
      then
        ...
        then
          echo "${INVT} WARNING: ${LOUD} Actual PMID count ${nm} does not match expected total ${num}${INIT}" >&2
        fi
      fi
      echo "$slr"
    elif [ -n "$ids" ]
